...
# paths
matrix_path = "../MDA/data/landen_samenwerking_matrix2024.csv"
category_path = "category_per_year"
project_path = "../MDA/data/cordis-HORIZONprojects-csv/project.csv"
euro_path = "../MDA/data/cordis-HORIZONprojects-csv/euroSciVoc.csv"
organization_path = "../MDA/data/cordis-HORIZONprojects-csv/organization.csv"
output_path = "/Users/mariejatsun/Downloads/country_network_2024_KNN5.json"
...
knn = NearestNeighbors(n_neighbors=K + 1, metric='euclidean')  # +1 omdat eerste buur het land zelf is
knn.fit(matrix.values)
distances, indices = knn.kneighbors(matrix.values)
...
for i, neighbors in enumerate(indices):
    c1 = country_list[i]
    for j in neighbors[1:]:  # sla zichzelf over
        c2 = country_list[j]
        pair = tuple(sorted((country_to_id[c1], country_to_id[c2])))
...
# select countries with at least 1 collaborations
landen_1plus = set(country_total_collab[country_total_collab >= 0].index)

print(f"\nAantal landen met minstens 1 samenwerkingen: {len(landen_1plus)}")
print(f"Landen: {sorted(landen_1plus)}")

# determine most common category for these countries
categorie_per_land = {}
for land in landen_1plus:
...
print("\nAantal landen per meest voorkomende categorie (voor landen met >=1 samenwerkingen):")
